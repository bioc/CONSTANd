%\VignetteIndexEntry{CONSTANd mass spectrometry}
%\VignetteKeywords{normalization mass-spectrometry}
%\VignetteDepends{CONSTANd}
%\VignettePackage{CONSTANd}

\documentclass[10pt]{article}

\title{Data normalization by matrix raking}
\author{
        Dirk Valkenborg, Jef Hooyberghs, Joris Van Houtven
}
...
\section{CONSTd Introduction}
The package \Rpackage(CONSTANd) is designed to ...

The package \Rpackage(CONSTANd) will perform the following function:
\begin{enumerate}
\item
Do something
\item
...
The data matrix is an $m \times n$ matrix...
...
\subsection{CONSTANd Function Details}


\subsubsection{Description of Results}


\section{CONSTANd Function Demonstration}
...
No libraries are required to use this package.


\subsection{Loading CONSTANd library}
