...
clean_organs <- function(organs) {
  ...
  organs <- lapply(organs, function(x) as.data.frame(x %>% group_by(Sequence) %>% summarise_at(.vars = quanCols, .funs = median, na.rm=T) %>% drop_na(., Sequence)))
  ## keep only quantification and peptide info; set rownames to peptide sequences
  for (i in 1:length(organs)) { rownames(organs[[i]]) <- organs[[i]]$Sequence; organs[[i]]$Sequence <- NULL}
  ...
  for (i in 1:length(organs)) { organs[[i]][is.na(organs[[i]])] <- 0 }
  ## remove rows with too many missing values by using median
  organs <- lapply(organs, function(x) x[apply(x, 1, median, na.rm = F) > 0,])
  ## set NA values again for use with CONSTANd
  for (i in 1:length(organs)) { organs[[i]][organs[[i]]==0] <- NA }
  return(organs)
}
...
rearrange_organs_design <- function(study.design) {
  ...
  tbl <- tbl %>% unite(uniqueChannel, c(run,channel), remove = F)
