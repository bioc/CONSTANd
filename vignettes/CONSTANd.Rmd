...
To warrant that the normalization procedure itself is not biased, three assumptions about the data are made (as for any type of data-driven normalization method) which may be verified by making MA-plots (see \@ref(s:maplots)):
...
```{r}
MAplot <- function(x,y,use.order=F, R=NULL, cex=1.6, showavg=T) {
...
When verifying that input data adheres to the assumptions in \@ref(s:assumptions), one could use this function (preferably on each pair of biological conditions in each IPS) without optional arguments:
...
If, for some reason, you do not want to re-scale the normalized data using the R vector, you can also use `use.order=T` option to make the A axis (horizontal axis) display the data in the order they appear in the `MAplot` function inputs. In that case, the `R` argument has no effect.
