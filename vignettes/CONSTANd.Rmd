...
organs.df <- Reduce(function(x, y) { tmp <- merge(x, y, by = 0, all = FALSE); rownames(tmp) <- tmp$Row.names; tmp$Row.names <- NULL; return(tmp) }, organs)
organs.norm.df <- Reduce(function(x, y) { tmp <- merge(x, y, by = 0, all = FALSE); rownames(tmp) <- tmp$Row.names; tmp$Row.names <- NULL; return(tmp) }, organs.norm)
```
